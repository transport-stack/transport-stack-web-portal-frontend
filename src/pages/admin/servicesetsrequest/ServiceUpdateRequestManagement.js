...
import { getChangeRequestStatusFilterAPI, getServicesetChangeRequestLists } from '../../../services/apiservices';
import SelectComponent from '../../../components/common/ui/select/SelectComponent';

const ServiceUpdateRequestManagement = () => {
  ...
  const columns = React.useMemo(() => [
    {
      ...
      Cell: (e) => (
        <button
          className='has-link'
          onClick={() => navigate("/admin/requestmanagement/viewservicesetupdaterequest/" + e.cell.row.original.requestId)}
        >
          {e.value}
        </button>
      ),
    ...
    },
    {
      ...
      Cell: (e) => (
        <button
          className='has-link'
          onClick={() => navigate("/admin/usermanagement/viewuser/" + e.cell.row.original.userId)}
        >
          {e.value}
        </button>
      ),
    },
    { Header: "Service ID", accessor: "resourceSequenceId" },
    { Header: "Service Title", accessor: "resourceName" },
    { Header: "Request Date", accessor: "creationDate", sortKey: "Inactive" },
    { Header: "Status", accessor: "displayStatus" },
  ...
  const fetchData = () => {
    ...
    let params =
      ...
      filterValue1;
    const getData = async () => {
      return await getServicesetChangeRequestLists(params);
    };
    getData().then((items) => {
  ...
  const fetchStatusMaster = () => {
    ...
    const getStatus = async () => {
      return await getChangeRequestStatusFilterAPI();
    };
...
export default ServiceUpdateRequestManagement;
