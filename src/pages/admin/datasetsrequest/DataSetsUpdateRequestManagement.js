...
import { getChangeRequestStatusFilterAPI, getDatasetChangeRequestLists } from '../../../services/apiservices';
import SelectComponent from '../../../components/common/ui/select/SelectComponent';

const DataSetsUpdateRequestManagement = () => {
  ...
  const columns = React.useMemo(() => [
    {
      ...
      Cell: (e) => (
        <button
          className='has-link'
          onClick={() => navigate("/admin/requestmanagement/viewdatasetupdaterequest/" + e.cell.row.original.requestId)}
        >
          {e.value}
        </button>
      ),
    ...
    { Header: "Data Set ID", accessor: "resourceSequenceId" },
    { Header: "Data Set Title", accessor: "resourceName" },
  ...
  const fetchData = () => {
    ...
    let params =
      ...
      filterValue1;
    const getData = async () => {
      return await getDatasetChangeRequestLists(params);
    };
    getData().then((items) => {
...
export default DataSetsUpdateRequestManagement;
