...
import { getDatasetLists, getStatusFilterAPI } from '../../../services/apiservices';
import SelectComponent from '../../../components/common/ui/select/SelectComponent';

const DataSetsRequestManagement = () => {
        ...
        () => [
            {
                ...
                Cell: (e) => (
                    <button className='has-link'
                        onClick={() =>
                            navigate("/admin/requestmanagement/viewdatasetrequest/" + e.cell.row.original.id)
                        }
                    >
                        {e.value}
                    </button>
            ...
            { Header: "Data Set ID", accessor: "dataSetId" },
            { Header: "Data Set Title", accessor: "dataSetName" },
    ...
    const fetchData = () => {
        ...
        let params =
            ...
            filterValue1;
        const getData = async () => {
            return await getDatasetLists(params);
        };
        getData().then((items) => {
...
export default DataSetsRequestManagement;
